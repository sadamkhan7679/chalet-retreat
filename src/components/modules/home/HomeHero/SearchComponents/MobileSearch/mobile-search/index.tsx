...
export default function MobileSearchStepper() {
  ...
  const selectedAmenities = useSelector(getSelectedAmenities).filter(
    (amenity: any) => {
  ...
  const getNavButtonStatus = () => {
    ...
    if (
      ...
      return true
    if (activeNav === 3 && selectedAmenities.length === 0) return true
  ...
  return (
    <div className={classes.root}>
      <Box className={classes.navBtnWrapper}>
        ...
        <Button
          onClick={handleNext}
          className={classes.navBtnNext}
          endIcon={<ArrowForwardIosIcon />}
          disabled={getNavButtonStatus()}
        >
      ...
      {activeNav === 0 && (
        <LocationSearchBox inTopBar={inTopBar} inMobileView={inMobileView} />
      )}

      {activeNav === 1 && (
        <Box
        //className={classes.mobileDateWrapper}
        >
