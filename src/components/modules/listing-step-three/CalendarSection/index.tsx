...
const CalendarSection: React.FC = () => {
  ...
  // const propertyId = property.id || 32;
  ...
  const handleImportIcal = (icalFeed: IcalFeedValues) => {
    addIcalFeedApi(icalFeed, propertyId).then((result) => {
      icalFeeds.push(result)
      getIcalFeedsApi(propertyId).then((data) =>
        setValue('ical_feeds', [...data]),
      )
      setPlatforms([...icalFeeds])
    })
  }

  const handleDeleteIcal = (index: number) => {
    setPlatforms((prevPlatforms) => {
      const platforms = [...prevPlatforms]
      const removedPlatforms = platforms.splice(index, 1)
      icalFeeds.splice(index, 1)
      setValue('ical_feeds', [...icalFeeds])

      if (removedPlatforms.length) {
        deleteIcalFeedApi(removedPlatforms[0].id)
      }
  ...
  const handleEditPricing = (calendarId: number) => {
    setCalendarId(calendarId)
    setModal('edit-pricing')
  }

  const handleDeletePricing = () => {
    deleteCalendarApi(calendarId).then(() => {
            ...
            <div className={classes.menuWrapper}>
              {platforms.map((icalFeed: IcalFeedValues, index: number) => (
                <ICalMenu
                  platform={icalFeed}
                  key={index}
                  onDelete={() => handleDeleteIcal(index)}
                />
              ))}
            </div>
          ) : (
