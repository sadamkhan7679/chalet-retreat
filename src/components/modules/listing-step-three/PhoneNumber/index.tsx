...
const PhoneNumber = () => {
  ...
  const forGuestNumber = phoneNumbers
    ? phoneNumbers.filter(
        (phoneNumber: any) =>
          !!phoneNumber.is_for_guests && phoneNumber.verified,
      )
    : null
  ...
  useEffect(() => {
    if (forGuestNumber && forGuestNumber.length > 0) {
      router.push(buildListingPath(next))
    }
  }, [forGuestNumber])
  ...
  return (
    <>
      <PhoneNumberValidationDialog
        ...
        phone_number={
          !primaryVerified ? primaryPhoneNumber : secondaryPhoneNumber
        }
      />
      <Container
