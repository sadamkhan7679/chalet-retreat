...
import Messages from 'src/components/modules/Messages'
...
import AccountSettingsFooter from './../AccountSetting/Footer/index'
...
const TabPanel = (props: any) => {
  ...
  return (
    ...
    >
      <Box p={3} style={{padding: index === 3 ? '0px' : undefined}}>
        {children}
      </Box>
    </Typography>
  )
...
const Header: React.FC<Props> = ({appBarProps, type}) => {
  ...
  const [payoutSection, setPayoutSection] = useState(false)
        ...
        <div className={classes.root} style={{background: 'white'}}>
          {!payoutSection && (
            <AppBar position="static" className={classes.headBar}>
              <Toolbar
                style={{
                  marginTop: isSmallSize ? '0px' : undefined,
                  padding: isSmallSize ? '0px 24px' : undefined,
                }}
              >
                {isSmallSize ? (
                  <Box
                  ...
                  >
                    ...
                    >
                      ...
                      <Divider
                        ...
                        variant="middle"
                        flexItem
                        style={{margin: '8px 6px 8px 10px', height: '24px'}}
                      />
                      <Box>
                        <Button className={classes.buttonAdd}>
                          <img src="/images/plus-line.svg" alt="plus-icon" />
                          <Typography
                            variant="body2"
                            style={{color: '#364259', fontWeight: 500}}
                          >
                            Add a property
                          </Typography>
                        </Button>
                        ...
                        <AppBar position="static" color="default">
                          ...
                          >
                            ...
                            <Tab
                              label="Bookings"
                              {...a11yProps(2)}
                              className={`${classes.tabInbox} ${classes.tabBooking}`}
                            />
                ...
                <Box
                  ...
                  className={
                    menuDrawer
                      ? `${classes.menuMobile} ${classes.showMenu}`
                      : `${classes.menuMobile}`
                  }
                >
                  <Box
          ...
          <AccountSettingsFooter setTabValue={setValue} />
        </div>
      ) : (
        ''
      )}
