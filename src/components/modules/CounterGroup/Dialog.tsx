...
const CounterGroupDialog: React.FC<Props> = ({
  ...
  const handleSave = () => {
    ...
    setBedCount(beds)
    if (roomData.id) {
      updateRoomsData({...roomData}).then((result) => {
        setValue(`rooms.${id}`, result.data)
      })
    } else {
      addRoomsData({...roomData, propertyId: property.id}).then((result) => {
        setValue(`rooms.${id}`, result.data)
      })
    }
    onClose()
  ...
  return (
    <AppDialog classes={{paper: classes.paper}} maxWidth="sm" open={open}>
      ...
      <Counter
        ...
        value={
          roomData.sleeping_types
            ? roomData.sleeping_types['King size'] || 0
            : 0
        }
      ...
      <Counter
        ...
        value={
          roomData.sleeping_types
            ? roomData.sleeping_types['Queen size'] || 0
            : 0
        }
      ...
      <Counter
        ...
        value={
          roomData.sleeping_types
            ? roomData.sleeping_types['Bunk beds'] || 0
            : 0
        }
      ...
      <Counter
        ...
        value={
          roomData.sleeping_types
            ? roomData.sleeping_types['Sleeping mattress'] || 0
            : 0
        }
        onChange={(count) =>
      ...
      <Counter
        ...
        label="Sofa bed"
        value={
          roomData.sleeping_types ? roomData.sleeping_types['Sofa bed'] || 0 : 0
        }
      ...
      <Counter
        ...
        label="Couch"
        value={
          roomData.sleeping_types ? roomData.sleeping_types['Couch'] || 0 : 0
        }
