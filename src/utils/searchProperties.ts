...
export function getSearchParams(page: number) {
  ...
  const params = {
    ...(location?.address &&
      location?.address !== '' && {full_address: location?.address}),
    ...(dateRange.startDate &&
    ...
    ...(location.addressComps?.city &&
      location.addressComps?.city !== '' && {
        city: location.addressComps?.city,
      }),
    ...(location.addressComps?.country &&
      location.addressComps?.country !== '' && {
        country: location.addressComps?.country,
      }),
    ...(location.addressComps?.state &&
      location.addressComps?.state !== '' && {
        state: location.addressComps?.state,
      }),
    ...(moreFilters?.beds &&
    ...
    ...(amenitySlugs?.length &&
      amenitySlugs?.length > 0 && {amenities: amenitySlugs}),
    is_active: 1,
    is_published: 1,
    page: page,
  }
